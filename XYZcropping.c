...
int croap(double xp[], double yp[], double zp[], int n, int onum, double H){
...
void perspective_polygon(double xp[100], double yp[100], double zp[100], int np, int onum, int poly)
{
  ...
  np=croap(xp, yp, zp, np, onum, H);
  
  for(int i =0; i<np; i++){
    xbb[i]=(400/H)*(cropx[i]/cropz[i])+400;
    ybb[i]=(400/H)*(cropy[i]/cropz[i])+400;
  }
